...
class Auth extends CI_Controller
{
	...
	function login()
	{
		...
		if ($this->tank_auth->is_logged_in()) {									// logged in
			if($this->input->post('rdr') != ''){
				redirect('Admin_controller/putorder/'.$this->input->get('rdr'));
			}
			else{
				//redirect('');
	...
	function register()
	{
		...
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			{
				//print_r($this->form_validation->set_value('user_type'));
				// validation ok
				if (!is_null($data = $this->tank_auth->create_user(
						...
						$this->form_validation->set_value('image_path')))) 
						{	// success
							
							
							if($this->input->post('user_type') == 'driver'){
								$this->load->model('Admin_model');
								$this->Admin_model->insert_driver($this->form_validation->set_value('firstname'),$this->form_validation->set_value('lastname'),$this->form_validation->set_value('mobile'),$data['user_id'],0);
							}


						if ($_FILES['image_path']['size'] <= 10000000) {

				            $result = $this->User_model->do_upload('image_path',$data['user_id']);
	...
	function register2()
	{
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			echo $email_activation = $this->config->item('email_activation', 'tank_auth');
			...
			if ($this->form_validation->run()) 
			{
				...
				if (!is_null($data = $this->tank_auth->create_user(
					...
					$this->User_model->serviceprovider($data); 
					$data['site_name'] = $this->config->item('website_name', 'tank_auth');
			...
			$data['captcha_registration'] = $captcha_registration;
			$data['use_recaptcha'] = $use_recaptcha;
			$this->load->view('auth/register_form2', $data);
		}
	...
	function activate()
	{
		...
		if ($this->tank_auth->activate_user($user_id, $new_email_key)) {		// success
			$udata = array('driver_status' => 1);
			$this->db->where('driver_id',$user_id);
			$this->db->update('service_providers_list',$udata);
